...
void FrustumData::applyWall( const fabric::Wall& wall )
{
    ...
    const Vector3f w = glm::normalize( glm::cross( u, v ));

    _type   = wall.type;
    _width  = glm::length( u );
    _height = glm::length( v );

    float* array = glm::value_ptr( _xfm );
    array[0]  = u[0];
    array[1]  = v[0];
    array[2]  = w[0];
    array[3]  = 0.;
    ...
    array[12] = -glm::dot( u, center );
    array[13] = -glm::dot( v, center );
    array[14] = -glm::dot( w, center );
    array[15] = 1.;
}
